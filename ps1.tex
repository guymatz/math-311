...
\\
Now suppose $x \in f^*(B_0) \cap f^*(B_1)$.  I.e. there is a $y_0 \in A_0 \cap A_1$ such that $f(x) = y_0)$ and $y_1 \in A_0 \cap A_1$ such that $f(x) = y_1)$.  Since $f$ is well-defined, it must be that $y_0 = y_1$, hence $y_0 \in A_0 \cap A_1$, and we can conclude that $x \in f^*(A_0 \cap A_1)$
\\
% c
...
\\
Now suppose $x \in \overline{f^*(B)}$. For $y \in B, f(x) \neq y$.  Since $f$ is a well-defined function, there exists $y_0 \in Y$ such that $f(x) = y_0$.  However, $y_0$ cannot be an element of B, so $y_0 \in \bar{B}$, and $x \in f^*(\bar{B})$.
...
Let $x \in X$ .  We want to show that $\forall y \in f^*(X), \exists! x:f(x) = y.$  $x$ exists since $y \in f^*(X)$ and is unique since $f$ is injective.  Now define $g_l : Y \to X$ as $g_l(y) = x_0$ if $y \notin f^*(X)$, otherwise, $g_l(y) = x$.  Since $f(x) \in f^*(X)$ we have $g_l(f(x)) = x$, and thus $(g_l \circ f)(x) = Id_X(x)$
...
Fix($f$), Fix($g$) are cofinite.  By Problem 11, Fix($f$) $\cap$ Fix($g$) is cofinite . . ?
\\
\item Show that if $f \in S_X$ is cofinite, so is $f^{-1}$.\\
\\
Let $x \in Fix(f)$, then, by definition $x \in Fix(f^{-1})$ and thus $Fix(f) \subseteq Fix(f^{-1})$.  A cofinite subset implies a cofinite superset, so $f^{-1}$ is cofinite.
\end{enumerate}
% end Q11
...
\newpage
% Q14
\item All of the parts of this problem have one-line answers
\begin{enumerate}
\item Show that $O(X)$ is conjugate-invariant\\
The results of 9a, above, show that $O(X)$ is conjugate-invariant
\\
\item Show that $T(X)$ is conjugate-invariant\\
The results of 9b, above, show that $T(X)$ is conjugate-invariant
\\
\item Show that $S(X)$ is conjugate-invariant\\
The results of 9c, above, show that $S(X)$ is conjugate-invariant
\\
\item Show that $Co(X)$ is conjugate-invariant\\
The results of 11a, above, show that $Co(X)$ is conjugate-invariant
\\
\item Show that $Co(X) \cap S_X$ is conjugate-invariant\\
The results of 11b, above, show that $Co(X) \cap S_X$ is conjugate-invariant
\\
\end{enumerate}
% end Q 14
